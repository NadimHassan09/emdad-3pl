...
async function main() {
  ...
  // Use raw SQL to clean up all tables (safer than checking each model)
  try {
  ...
  // 12. Create Inventory Ledger Entries (which will auto-update current_stock via trigger)
  console.log('📊 Creating inventory ledger entries...');
  
  // Receipt for product1, batch1 in warehouse1, location1
  ...
  // Receipt for product1, batch2 in warehouse1, location2
  await prisma.inventoryLedger.create({
    data: {
      clientId: client1.id,
      warehouseId: warehouse1.id,
  ...
  // Receipt for product2, batch3 in warehouse1, location1
  await prisma.inventoryLedger.create({
    data: {
      clientId: client1.id,
      warehouseId: warehouse1.id,
  ...
  // Receipt for product3 in warehouse2, location3
  await prisma.inventoryLedger.create({
    data: {
      clientId: client2.id,
      warehouseId: warehouse2.id,
